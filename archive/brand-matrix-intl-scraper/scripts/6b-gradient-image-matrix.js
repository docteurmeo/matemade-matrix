...
async function getImages(brandId, max) {
  if (!max) max = 4;
...
function layoutAxis(points) {
  const sorted = points.map(function (p, idx) {
    return { id: p.id, pos: p.pos, origIdx: idx };
  }).sort(function (a, b) { return a.pos - b.pos || a.origIdx - b.origIdx; });

  const xs = sorted.map(function (p) { return LEFT_MARGIN + (p.pos / 100) * GUESS_WIDTH; });
...
async function buildAxisHtml(axis, brandsById, analysisById, imageCache) {
  const layout = layoutAxis(axis.points);
  ...
  for (const pt of items) {
    ...
    if (imgs.length === 0) {
      ...
      const tiles = shown.map(function (src) { return '<img src="' + src + '" alt="' + esc(b.name) + '" loading="lazy">'; }).join('');
      collage = '<div class="collage ' + gridClass + (isMate ? ' mate' : '') + '" style="width:' + size + 'px;height:' + size + 'px;">' + tiles + '</div>';
    }

    const tickStyle = pt.side === 'above'
      ? ('height:' + TICK_GAP + 'px;top:-' + TICK_GAP + 'px;')
      : ('height:' + TICK_GAP + 'px;top:1px;');

    html.push(
      '<div class="point" style="left:' + pt.x + 'px;" data-side="' + pt.side + '">' +
        '<div class="tick" style="' + tickStyle + '"></div>' +
        collage +
...
async function main() {
  const brandsData = JSON.parse(await fs.readFile(BRANDS_FILE, 'utf8'));
  ...
  const missing = [...neededIds].filter(function (id) { return (imageCache[id] || []).length === 0; });
  ...
  const html =
    ...
    (missing.length ? '  <div class="missing-note">⚠️ Chưa có ảnh sản phẩm đã scrape cho: ' + missing.map(function (id) { return esc((brandsById[id] && brandsById[id].name) || id); }).join(', ') + ' — các brand này hiện hiển thị placeholder (chữ cái đầu) thay cho collage ảnh.</div>\n' : '') +
    '</div>\n\n' +
    '<main class="main">\n' +
    axisSections.join('\n') + '\n' +
    '</main>\n\n' +
